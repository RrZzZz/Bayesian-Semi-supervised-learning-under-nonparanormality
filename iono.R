...
for(idx in 1:10){
  ...
  fp[idx]=sum(r[L.test==0]<1)/sum(L.test==0)
  fn[idx]=sum(r[L.test==1]>1)/sum(L.test==1)
  error[idx]=(sum(r[L.test==0]<1)+sum(r[L.test==1]>1))/length(L.test)
...
save.image('//wolftech.ad.ncsu.edu/cos/stat/Redirect/rzhu4/Downloads/research3/real data/iono-new')
